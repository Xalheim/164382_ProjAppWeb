...
class Store {
	...
	function StorePage() {
		...
		if (isset($_GET['add_produkt'])) {
			echo $this->AddToCart($_GET['add_produkt']);
		}
	...
	function ShowCart() {
		...
		if (isset($_GET['clear'])) {
			echo $this->ClearCart();																											// Wyczyść koszyk oraz count
		}
		if (isset($_GET['add_one'])) {
			echo $this->AddOne($_GET['add_one']);																								// Dodaj ilość 1 do produktu
		}
		if (isset($_GET['remove_one'])) {
			echo $this->RemoveOne($_GET['remove_one']);																						// Usun 1 z produktu, ewentualnie cały produkt
		}
	}
