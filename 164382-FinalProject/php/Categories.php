...
class Category {	
	...
	function AddCategory() {
		...
		if ($status_login == 1) {																// Czy pomyślnie się zalogowano?
			...
			if (isset($_POST['create_mother']) && isset($_POST['create_name'])) {		// czy zostały podane wszystkie dane?
				$_POST['create_id'] = htmlspecialchars($_POST['create_id']);
				$_POST['create_mother'] = htmlspecialchars($_POST['create_mother']);
				$query = "INSERT INTO category_list (matka, nazwa)
						  VALUES ('".$_POST['create_mother']."', '".$_POST['create_name']."') LIMIT 1";	// Wprowadź nowe dane do tablicy category_list
				...
				echo $this->DodajKategorie();												// Pobierz dane do Dodania
			}
		}
		else {
			echo $Admin->FormularzLogowania();												// Dane się nie zgadzają lub nie zostały podane, wyświetla ponownie formularz logowania.
	...
	function EditCategory() {
		...
		if ($status_login == 1) {													// Czy pomyślnie się zalogowano?
			...
			if (isset($_POST['edit_id']) && isset($_POST['edit_mother']) && isset($_POST['edit_name'])) {
				...
				echo $this->EdytujKategorie();												// Pobierz dane do edycji
			}
		}
		else {
			echo $Admin->FormularzLogowania();												// Dane się nie zgadzają lub nie zostały podane, wyświetla ponownie formularz logowania.
	...
	function DeleteCategory() {
		...
		if ($status_login == 1) {													// Czy pomyślnie się zalogowano?
			...
			if (isset($_POST['delete_id'])) {
				...
				echo $this->UsunKategorie();												// Pobierz ID do usunięcia
			}
		}
		else {
			echo $Admin->FormularzLogowania();												// Dane się nie zgadzają lub nie zostały podane, wyświetla ponownie formularz logowania.
